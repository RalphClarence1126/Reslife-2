...
<div class="padded-top-bottom border-top">
	<div class="padded-left-right">
		...
		$total_submissions = 1;
		$item_range = $filter_range * ($_SESSION['page_number'] + 1);
		$item_iterator = ($item_range - $filter_range) + 1;
		if ($submissions) {
			if (mysqli_num_rows($submissions) > 0) {
				while ($admissions = $submissions->fetch_assoc()) {
					...
					// 		include('submission_student.php');
					// 	}
					// }

					if ($admissions_status !== 'PENDING') {
						if ($item_iterator <= $item_range) {
							$_where_filter = (!$where_filter) ? "WHERE stds_submission_id = '$item_iterator'" : "WHERE stds_submission_id = '$item_iterator' AND $join_filter";

							$_sql = "SELECT * FROM stds_frm_addm $_where_filter ORDER BY $filter_type $filter_order";
							$_query = mysqli_query($mysqli, $_sql);
							while ($_row = $_query->fetch_assoc()) {
								$admissions_id = $_row['stds_submission_id'];
								$admissions_student_account_id = $_row['stds_acc_id'];
								$admissions_student_number = $_row['stds_std_number'];
								$admissions_student_lrn = $_row['stds_std_lrn'];
					...
					$_SESSION['pagination_check'] = $total_submissions % $filter_range;
					$_SESSION['pagination_number'] = intval($total_submissions / $filter_range);
				}

				// $submissions->free();

				include('submission_pagination.php');
			} else {
		...
		if ($submissions) {
			if (mysqli_num_rows($submissions) > 0) {
				while ($admissions = $submissions->fetch_assoc()) {
					...
					($total_submissions <= $filter_range) ? include('submission_student.php') : '';
